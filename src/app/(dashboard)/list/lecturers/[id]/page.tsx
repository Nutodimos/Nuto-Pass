        ...
        <div className="w-full xl:w-2/3 flex flex-col gap-4">
              ...
              <div className="flex-1 text-white min-w-0">
                <div className="flex flex-row items-start md:items-center justify-between gap-2 mb-2 md:mb-4">
                  <div className="flex flex-col min-w-0">
                    ...
                    <span className="text-sm text-white/80 font-medium truncate">ID: {teacher.username}</span>
                  </div>
                  {role === "admin" && (
                ...
                <div className="flex overflow-x-auto pb-2 -mx-2 px-2 md:mx-0 md:px-0 md:pb-0 hide-scrollbar gap-2 md:grid md:grid-cols-2 lg:grid-cols-4 w-full text-left mt-2 md:mt-4">
                  <div className="flex-shrink-0 flex items-center gap-2 text-xs md:text-sm text-white/90 bg-white/10 md:bg-transparent px-3 py-1.5 md:p-0 rounded-full md:rounded-none">
                    ...
                    <span className="truncate max-w-[150px] md:max-w-none">{teacher.email || "-"}</span>
                  </div>
                  ...
                  <div className="flex-shrink-0 flex items-center gap-2 text-xs md:text-sm text-white/90 bg-white/10 md:bg-transparent px-3 py-1.5 md:p-0 rounded-full md:rounded-none">
                    ...
                    <span>{teacher.birthday ? new Intl.DateTimeFormat("en-GB").format(teacher.birthday) : "-"}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="xl:hidden group cpe-card p-4">
            ...
            <div className="flex overflow-x-auto pb-2 -mx-2 px-2 hide-scrollbar gap-2 relative z-10">
              <Link className="flex-shrink-0 px-4 py-2 rounded-xl bg-CPENavy text-white text-sm font-medium hover:bg-CPENavyDark transition-colors" href={`/list/lessons?teacherId=${teacher.id}`}>Lessons</Link>
              <Link className="flex-shrink-0 px-4 py-2 rounded-xl bg-CPEGold text-white text-sm font-medium hover:bg-CPEGoldDark transition-colors" href={`/list/students?teacherId=${teacher.id}`}>Students</Link>
              <Link className="flex-shrink-0 px-4 py-2 rounded-xl bg-CPESlate text-white text-sm font-medium hover:bg-CPENavy transition-colors" href={`/list/assignments?teacherId=${teacher.id}`}>Assignments</Link>
              <Link className="flex-shrink-0 px-4 py-2 rounded-xl bg-CPEGoldLight text-white text-sm font-medium hover:bg-CPEGold transition-colors" href={`/list/materials?teacherId=${teacher.id}`}>Materials</Link>
            </div>
          </div>

          {/* STATS CARDS */}
          <div className="grid grid-cols-2 gap-4">
            <Link href={`/list/courses?teacherId=${teacher.id}`} className="bg-gradient-to-br from-CPESlate/20 to-CPENavy/10 p-4 rounded-2xl border border-CPENavy/20 flex flex-col sm:flex-row sm:items-center gap-3 hover:shadow-lg transition-all hover:scale-[1.02] cursor-pointer">
              <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-CPENavy flex items-center justify-center shrink-0">
                <BookOpen className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
              </div>
              <div>
                <h3 className="text-xl sm:text-2xl font-bold text-CPENavyDark">{teacher._count.subjects}</h3>
                <p className="text-xs sm:text-sm text-CPENavy">Courses</p>
              </div>
            </Link>

            <Link href={`/list/lessons?teacherId=${teacher.id}`} className="bg-gradient-to-br from-CPEGoldLight/20 to-CPEGold/10 p-4 rounded-2xl border border-CPEGold/20 flex flex-col sm:flex-row sm:items-center gap-3 hover:shadow-lg transition-all hover:scale-[1.02] cursor-pointer">
              <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-CPEGold flex items-center justify-center shrink-0">
                <Calendar className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
              </div>
              <div>
                <h3 className="text-xl sm:text-2xl font-bold text-CPEGoldDark">{teacher._count.lessons}</h3>
        ...
        <div className="w-full xl:w-1/3 flex flex-col gap-4">
          ...
          <div className="hidden xl:block group cpe-card p-5">
            ...
            <div className="flex flex-wrap gap-2 relative z-10">
              <Link className="px-4 py-2 rounded-xl bg-CPENavy text-white text-sm font-medium hover:bg-CPENavyDark transition-colors" href={`/list/lessons?teacherId=${teacher.id}`}>Lessons</Link>
              <Link className="px-4 py-2 rounded-xl bg-CPEGold text-white text-sm font-medium hover:bg-CPEGoldDark transition-colors" href={`/list/students?teacherId=${teacher.id}`}>Students</Link>
              <Link className="px-4 py-2 rounded-xl bg-CPESlate text-white text-sm font-medium hover:bg-CPENavy transition-colors" href={`/list/assignments?teacherId=${teacher.id}`}>Assignments</Link>
              <Link className="px-4 py-2 rounded-xl bg-CPEGoldLight text-white text-sm font-medium hover:bg-CPEGold transition-colors" href={`/list/materials?teacherId=${teacher.id}`}>Materials</Link>
            </div>
          </div>

          {/* DESKTOP ACADEMICS (Hidden on mobile since elevated left) */}

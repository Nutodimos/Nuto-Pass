        ...
        <div className="max-w-6xl mx-auto p-4 md:p-6 space-y-8">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div className={`px-4 py-2 rounded-xl flex items-center gap-2 border font-bold text-xs uppercase tracking-widest ${
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                <StatCard 
                    label="Templates" 
                    value={`${device?.templateCount || 0} / 127`} 
                    subValue={`${Math.round(((device?.templateCount || 0) / 127) * 100)}% capacity`}
                                    ...
                                    <div className="relative w-32 h-32 flex items-center justify-center">
                                        <svg className="w-full h-full transform -rotate-90">
                                            ...
                                            <circle 
                                                ...
                                                strokeDasharray={377}
                                                strokeDashoffset={377 - (377 * (device?.templateCount || 0)) / 127}
                                                className="text-CPEGold transition-all duration-1000"
                                            />
                                        </svg>
                                        <div className="absolute inset-0 flex flex-col items-center justify-center">
                                            <span className="text-2xl font-black">{device?.templateCount || 0}</span>
                                            <span className="text-[10px] font-bold text-white/50 uppercase">Stored</span>
                                        </div>
                                    </div>
                                    <div className="flex-1 space-y-2 text-center md:text-left">
                                        <h4 className="text-xl font-bold">R307 Memory Status</h4>
                                        <p className="text-white/60 text-sm leading-relaxed max-w-md">
                                            The fingerprint sensor can store up to 127 unique templates. Currently, {device?.templateCount || 0} slots are occupied.
                                        </p>
                                    </div>
                                </div>
                            </div>
